define([
  ...
  class TableSelectedChangeChain extends ActionChain {
    ...
    async run(context, { event, previousValue, value, updatedFrom, keys, selected }) {
      ...
      let selectiondata=[];
      if (isAllselected) {
        selectiondata = $variables.linesAdp.data;
      }else{
        if (selectedkeys) {
          const results = await ActionUtils.forEach(selectedkeys, async (item, index) => {
            const results2 = await ActionUtils.forEach($variables.linesAdp.data, async (items, indexs) => {
              if (item===items.line_id) {
      ...
      $variables.tableSelectedAdp.data = selectiondata;

      if (selectiondata.length !== 0) {
        const isdiabled = await $functions.checkStatus($variables.tableSelectedAdp.data);
        $variables.isBtnDisabled = isdiabled;
      }
      else {
        $variables.isBtnDisabled = true;
      }
